export const sysUserMfaRecoveryCodesApi = {
  ...
  page(data: SysUserMfaRecoveryCodesQuery) {
    return request.page<Result<SysUserMfaRecoveryCodes[]>>('/system/sys-user-mfa-recovery-codes/page', data)
  },
  // 查询MFA备用验证码列表
  list(data: SysUserMfaRecoveryCodesQuery) {
    return request.get<SysUserMfaRecoveryCodes[]>('/system/sys-user-mfa-recovery-codes/list', data)
  },
  // 根据ID查询MFA备用验证码详情
  getById(id: number) {
