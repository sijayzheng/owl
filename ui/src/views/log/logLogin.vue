<template>
  <div>
    ...
    <el-card class="table-card" shadow="hover">
      <template #header>
        <el-button v-hasPerm="['log:logLogin:add']" icon="Plus" plain type="primary" @click="dialogVisible = true">
          新增
        </el-button>
        <el-button v-hasPerm="['log:logLogin:remove']" icon="Delete" plain type="danger" @click="handleDelete()">
          删除
        </el-button>
...
const { loading: submitting, form, send: submit, reset: resetForm, updateForm } = useForm(
  formData => logLoginApi.save(formData),
  {
    initialForm: logLoginInitData,
...
function handleUpdate(id: number) {
  logLoginApi.getById(id).then((res) => {
    updateForm(res)
...
function handleDelete(id?: number) {
  ...
  if (ids.length > 0)
    modal.confirm(`是否确认删除登录日志编号为"${ids}"的数据项？`).then(() => logLoginApi.remove(ids).then(() => listData()))
  else
