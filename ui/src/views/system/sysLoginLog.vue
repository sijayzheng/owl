<template>
  <div>
    <el-card body-style="padding-bottom:8px" class="search-card" shadow="hover">
      <el-form ref="queryFormRef" :inline="true" :model="queryParams">
        ...
        <el-form-item label="登录ip地址" prop="loginIp">
          <el-input v-model="queryParams.loginIp" clearable placeholder="请输入登录ip地址" @keyup.enter="handleQuery" />
        </el-form-item>
        <el-form-item label="登录地址" prop="location">
          <el-input v-model="queryParams.location" clearable placeholder="请输入登录地址" @keyup.enter="handleQuery" />
        </el-form-item>
        <el-form-item label="浏览器类型" prop="browser">
          <el-input v-model="queryParams.browser" clearable placeholder="请输入浏览器类型" @keyup.enter="handleQuery" />
        </el-form-item>
        <el-form-item label="操作系统" prop="os">
          <el-input v-model="queryParams.os" clearable placeholder="请输入操作系统" @keyup.enter="handleQuery" />
        </el-form-item>
        <el-form-item label="登录状态" prop="succeeded">
          <el-radio-group v-model="queryParams.succeeded" @change="() => handleQuery()">
        ...
        <el-form-item label="提示消息" prop="message">
          <el-input v-model="queryParams.message" clearable placeholder="请输入提示消息" @keyup.enter="handleQuery" />
        </el-form-item>
    ...
    <el-card class="table-card" shadow="hover">
      <template #header>
        <el-button v-hasPerm="['system:sysLoginLog:add']" icon="Plus" plain type="primary" @click="dialogVisible = true">
          新增
        </el-button>
        <el-button v-hasPerm="['system:sysLoginLog:remove']" icon="Delete" plain type="danger" @click="handleDelete()">
          删除
        </el-button>
        <el-button v-hasPerm="['system:sysLoginLog:export']" :loading="exportDataLoading" icon="Download" plain type="warning" @click="exportData">
          导出
        </el-button>
        <el-button v-hasPerm="['system:sysLoginLog:export']" :loading="templateLoading" icon="Download" plain type="warning" @click="downloadTemplate">
          下载模板
        </el-button>
        <el-button v-hasPerm="['system:sysLoginLog:export']" :loading="uploading" icon="Upload" plain type="warning" @click="handleUpload">
          导入{{ progress }}
        </el-button>
      </template>
        ...
        <el-table-column align="center" fixed="right" label="操作" width="150">
          <template #default="{ row }">
            <el-tooltip content="修改" placement="top">
              <el-button v-hasPerm="['system:sysLoginLog:edit']" icon="Edit" link type="primary" @click="handleUpdate(row.id)" />
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button v-hasPerm="['system:sysLoginLog:remove']" icon="Delete" link type="primary" @click="handleDelete(row.id)" />
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 添加或修改登录日志对话框 -->
    <el-dialog v-model="dialogVisible" append-to-body title="保存登录日志" width="500px" @closed="() => resetForm()">
...
const tableRef = ref()
const queryParams = ref<SysLoginLogPageQuery>({
  page: 1,
  size: 10,
})
const dialogVisible = ref(false)

const { data, send: listData, total, loading } = usePagination(() => sysLoginLogApi.page(queryParams.value))
const { loading: submitting, form, send: submit, reset: resetForm, updateForm } = useForm(
  formData => sysLoginLogApi.save(formData),
  {
    initialForm: sysLoginLogInitData,
    resetAfterSubmiting: true,
  },
).onComplete(() => dialogVisible.value = false).onSuccess(() => listData())

const { loading: templateLoading, send: downloadTemplate } = sysLoginLogApi.downloadTemplate()
const { loading: exportDataLoading, send: exportData } = sysLoginLogApi.exportData(queryParams.value)
const { uploading, file, appendFiles, removeFiles, upload } = useUploader(({ file }) => sysLoginLogApi.importData(file), { limit: 1 }).onComplete(() => removeFiles())
...
function handleUpdate(id: number) {
  sysLoginLogApi.getById(id).then((res) => {
    updateForm(res)
...
function handleDelete(id?: number) {
  ...
  if (ids.length > 0)
    modal.confirm(`是否确认删除登录日志编号为"${ids}"的数据项？`).then(() => sysLoginLogApi.remove(ids).then(() => listData()))
  else
