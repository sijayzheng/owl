...
@RequestMapping("/file/fileStorage")
@RestController
public class FileStorageController extends BaseController {
    ...
    @GetMapping("/page")
    public Result<List<FileStorage>> page(PageQuery pageQuery, FileStorageQuery fileStorageQuery) {
        return success(fileStorageService.page(pageQuery, fileStorageQuery));
    }
    ...
    @SaCheckPermission("file:fileStorage:query")
    @GetMapping("/list")
    public Result<List<FileStorage>> list(FileStorageQuery fileStorageQuery) {
        return success(fileStorageService.list(fileStorageQuery));
    }
     ...
     * @return 操作结果
     */
    @AccessLog(title = "文件存储", operateType = OperateType.SAVE)
    @SaCheckPermission("file:fileStorage:save")
    @PostMapping("/save")
    public Result<Boolean> save(@Valid @RequestBody FileStorage fileStorage) {
        return result(fileStorageService.validSave(fileStorage), OperateType.SAVE);
    }
    ...
    @GetMapping("/export")
    public ResponseEntity<Resource> exportData(FileStorageQuery fileStorageQuery) {
        List<FileStorage> list = fileStorageService.list(fileStorageQuery);
