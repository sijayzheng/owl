...
(
    ...
    index idx_gen_table_table_name (table_name)
) comment '代码生成表信息';
...
(
    ...
    update_by    bigint       default 1 comment '更新者',
    update_time  datetime     default current_timestamp comment '更新时间',
    index idx_sys_dict_data_dict_type_id (dict_type_id)
) comment '字典数据表';
...
(
    ...
    index idx_sys_user_online_user_id (user_id),
...
(
    ...
    create_time datetime default current_timestamp comment '创建时间',
    index idx_sys_user_mfa_recovery_codes_user_id (user_id),
    index idx_sys_user_mfa_recovery_codes_code (code)
) comment 'MFA备用验证码表';
...
(
    ...
    update_by      bigint   default 1 comment '更新者',
    update_time    datetime default current_timestamp comment '更新时间',
    index idx_sys_notice_deleted (deleted)
) comment '通知公告表';
...
(
    ...
    update_by       bigint               default 1 comment '更新者',
    update_time     datetime             default current_timestamp comment '更新时间',
    index idx_sys_message_sender (sender),
    index idx_sys_message_recipient (recipient),
    index idx_sys_message_has_read (has_read),
    index idx_sys_message_create_time (create_time)
) comment '系统消息表';
...
(
    ...
    location   varchar(200) default '' comment '登录地址',
...
(
    ...
    index idx_file_storage_create_by (create_by),
    index idx_file_storage_create_time (create_time),
    index idx_file_storage_deleted (deleted)
) comment '文件存储表';
...
(
    ...
    update_time   datetime              default current_timestamp comment '更新时间',
    update_by     bigint                default 0 comment '更新人',
    index idx_file_oss_storage_create_by (create_by),
    index idx_file_oss_storage_create_time (create_time),
    index idx_file_oss_storage_deleted (deleted)
) comment 'OSS对象存储表';
...
(
    ...
    index idx_sys_task_task_key (task_key),
    index idx_sys_task_task_group (task_group),
    index idx_sys_task_enabled (enabled)
) comment '定时任务配置表';
...
(
    ...
    index idx_log_task_task_id (task_id),
    index idx_log_task_execution_id (execution_id),
    index idx_log_task_start_time (start_time),
    index idx_log_task_status (status)
) comment '定时任务执行日志表';
